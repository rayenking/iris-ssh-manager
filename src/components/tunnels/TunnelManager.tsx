...
function getTunnelLabel(tunnel: Tunnel) {
  if (tunnel.type === 'local') {
    return `127.0.0.1:${tunnel.localPort} → ${tunnel.remoteHost}:${tunnel.remotePort}`;
  }

  if (tunnel.type === 'remote') {
    return `remote:${tunnel.remotePort} → ${tunnel.localHost}:${tunnel.localPort}`;
  }
...
export function TunnelManager({ sessionId }: Props) {
  ...
  const { tunnels, isLoading, isCreating, error, createTunnel, stopTunnel, refresh } = useTunnel(sessionId);
    ...
    <div className="relative flex h-full w-[360px] shrink-0 flex-col border-l border-[var(--color-border)] bg-[var(--color-bg-secondary)]">
      <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
        <div>
          ...
          <p className="text-xs text-[var(--color-text-muted)]">Local, remote, and dynamic port forwarding</p>
        </div>
      ...
      {!sessionId ? (
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          {sortedTunnels.map((tunnel) => (
            <div key={tunnel.id} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-primary)] p-3 shadow-sm">
              ...
              <div className="flex items-center justify-between gap-4 text-xs text-[var(--color-text-muted)]">
                ...
                <button
                  type="button"
                  disabled={tunnel.status !== 'active'}
                  onClick={() => void stopTunnel(tunnel.id)}
                  className="inline-flex items-center gap-1 rounded px-2 py-1 text-[var(--color-text-secondary)] hover:bg-[var(--color-hover)] hover:text-[var(--color-text-primary)] disabled:cursor-not-allowed disabled:opacity-50"
                >
